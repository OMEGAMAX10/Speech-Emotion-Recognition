...
from utils import bytes_to_sound, preprocess_wav, predict_autoencoder, get_best_emotion
...
MODEL_PATH_AE = './Models/EMO-IIT_converted_log_AutoEncoder.h5'
model = keras.models.load_model(MODEL_PATH)
model_ae = keras.models.load_model(MODEL_PATH_AE)
            ...
            while True:
                ...
                if not data:
                    ...
                    for idx in range(0, len(spectrogram_list), emotion_list_onesec_length):
                        ...
                        emotions = predict_autoencoder(model_ae=model_ae, model_ser=model, spectrogram_list=np.array(spectrogram_list[idx: idx + emotion_list_onesec_length]))
                ...
                if len(audio_content) >= audio_length:
                    ...
                    emotions = predict_autoencoder(model_ae=model_ae, model_ser=model, spectrogram_list=spectrogram_list)
                    emotion_list.extend(emotions)
                    response = f"{second_for_processing - 1} {second_for_processing} {get_best_emotion(emotion_list[-emotion_list_onesec_length:], max_length_consecutives=int(MAX_LENGTH_CONSECUTIVES_PERCENTAGE / HOP_LENGTH_SECONDS))}\n"
                    self.conn.sendall(response.encode())
                    print(response, end='')

...
SPECTROGRAM_DIR = "Spectrograms/EmoDB_Separate_Speakers_v2/female/Converted_Log"
TF_RECORDS_DIR = "TFRecords/EmoDB_Separate_Speakers_v2_female_MobileNetV3Large"
TF_RECORDS_NAME = "EmoDB_Separate_Speakers_v2_female_converted_log_MobileNetV3Large.tfrecords"
MODEL_DIR = "Models"
MODEL_NAME = "EmoDB_Separate_Speakers_v2_female_converted_log_MobileNetV3Large.h5"
NUM_CLASSES = 7
SAMPLE_RATE = 8000
BATCH_SIZE = 32
EPOCHS = 50
...
def create_dataframe_emodb_separate_speakers_gendered():
    EMOTION_DICT_EMODB = {'W': 'anger', 'L': 'boredom', 'E': 'disgust', 'A': 'fear', 'F': 'happiness', 'T': 'sadness', 'N': 'neutral'}
    if DATASET_DIR != "Datasets/EmoDB" and DATASET_DIR != "Datasets/Converted Datasets/EmoDB":
    ...
    return {"male": male_dfs, "female": female_dfs}
...
GENDER = "female"
FOLD_TO_TRAIN = 4
ser_df_dict = create_dataframe_emodb_separate_speakers_gendered()
ser_df_parts = ser_df_dict[GENDER]
assert FOLD_TO_TRAIN < len(ser_df_parts), f"FOLD_TO_TRAIN must be less than the number of folds, which is {len(ser_df_parts)}!"
strategy = tf.distribute.MirroredStrategy()
...
for fold, test_df in enumerate(ser_df_parts):
    ...
    emotion_list_test_unique = test_df["emotion"].unique()
    ...
    if not os.path.exists(new_spectrogram_dir):
        ...
        os.makedirs(os.path.join(new_spectrogram_dir, "dev"))
        os.makedirs(os.path.join(new_spectrogram_dir, "test"))
    ...
    # create_spectrogram_dataset(audio_block_list_test, emotion_list_test, new_tfrecords_name, new_spectrogram_dir, sr=SAMPLE_RATE, create_spectrogram=create_spectrogram_log, sample_weight=test_sample_weight, dataset_type="test")
    ...
    if fold == FOLD_TO_TRAIN:
        ...
        del audio_block_list_train, emotion_list_train, audio_block_list_test, emotion_list_test

...
def RMSELoss_batch(y_true, y_pred):
    return tf.keras.backend.sqrt(tf.keras.losses.mean_squared_error(y_true, y_pred))
...
def create_ser_model():
    ...
    autoencoder.compile(optimizer=optimizer, loss=SSIMLoss, metrics=['accuracy', SSIMLoss])
    return autoencoder
...
ohe_test = joblib.load(os.path.join(MODEL_DIR, MODEL_NAME.replace(".h5", "_ohe.pkl")))
...
train_dataset = get_dataset_autoencoder(batch_size=BATCH_SIZE, dataset_type="train")
dev_dataset = get_dataset_autoencoder(batch_size=BATCH_SIZE, dataset_type="dev")
...
# convert all emotions that are not ['anger', 'irritation', 'fear'] to neutral
